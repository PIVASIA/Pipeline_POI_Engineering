...
# mlflow.set_experiment("/Users/hunglv@piv.asia/scene-text-recognition")
...
#        bboxes = subm[k].decode("utf-8").split('\n')
#        temp = []
#        for bbox in bboxes:
#            bbx = [int(bb) for bb in bbox.split(',')[:-1]]
#            coords = [(bbx[j], bbx[j+1]) for j in range(0, len(bbx), 2)]
#            print(coords)
#            if geometry.Polygon(coords).exterior == True:
#                temp.append(bbox)
#        print(temp)
#        if len(temp) > 0:
...
def evaluate_method(gtFilePath, submFilePath, evaluationParams):
    ...
    for resFile in gt:
        ...
        det_only_hmean = (
            ...
            else 2.0 * det_only_precision * det_only_recall / (det_only_precision + det_only_recall)
        )
    ...
    det_only_methodRecall = (
        0 if det_only_numGlobalCareGt == 0 else float(det_only_matchedSum) / det_only_numGlobalCareGt
    )
    det_only_methodPrecision = (
        0 if det_only_numGlobalCareDet == 0 else float(det_only_matchedSum) / det_only_numGlobalCareDet
    )
    det_only_methodHmean = (
        0
        if det_only_methodRecall + det_only_methodPrecision == 0
        else 2 * det_only_methodRecall * det_only_methodPrecision / (det_only_methodRecall + det_only_methodPrecision)
    )
    ...
    mlflow.log_metric("E2E_precision", methodPrecision)
    mlflow.log_metric("E2E_recall", methodRecall)
    mlflow.log_metric("E2E_hmean", methodHmean)
    
    mlflow.log_metric("DET_precision", det_only_methodPrecision)
    mlflow.log_metric("DET_recall", det_only_methodRecall)
    mlflow.log_metric("DET_hmean", det_only_methodHmean)
    resDict = {
        "calculated": True,
        "Message": "",
